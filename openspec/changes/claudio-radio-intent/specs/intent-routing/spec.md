...
The system SHALL classify incoming user messages into one of three intent types: "music_command", "switch_music_type", or "chat_message" based on content analysis.
...
#### Scenario: Switch music type detected
- **WHEN** the user sends "换点轻松的音乐" (switch to lighter music)
- **THEN** the router SHALL classify it as `switch_music_type`, clear the current queue, stop playback, and route to search with the extracted type keyword
...
#### Scenario: Switch music type routing
- **WHEN** intent is `switch_music_type`
- **THEN** the router SHALL clear the current queue, trigger a music search for the new type, generate a DJ transition message via Claude, and return both the response and the new queue with `replaceQueue: true`

#### Scenario: Chat routing
- **WHEN** intent is `chat_message`
- **THEN** the router SHALL trigger context assembly, call the Claude adapter, parse the response, execute any actions, and return the AI response without interrupting current playback
